...
public final class NativePRNG extends SecureRandomSpi {
    ...
    private static class RandomIO {
        ...
        private volatile provider.SecureRandom mixRandom;
        ...
        private provider.SecureRandom getMixRandom() {
            provider.SecureRandom r = mixRandom;
            if (r == null) {
                synchronized (LOCK_GET_BYTES) {
                    r = mixRandom;
                    if (r == null) {
                        r = new provider.SecureRandom();
                        try {
                            byte[] b = new byte[20];
